...
const compileElectronCode = function (javascriptCode, options) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      electronPath,
      [bytenodePath, "--compile", "--no-module", "-"],
      {
...
const compileFile = async function (args, output) {
  ...
  const compiledFilename =
    args.output ||
    output ||
    filename.slice(0, -path.extname(filename).length) + COMPILED_EXTNAME;

  if (typeof compiledFilename !== "string") {
    throw new Error(
      `output must be a string. ${typeof compiledFilename} was given.`
    );
  }
